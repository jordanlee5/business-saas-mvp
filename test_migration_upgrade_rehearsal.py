...
class MigrationUpgradeRehearsalTests(unittest.TestCase):
    ...
    def test_order_foundation_revision_can_rehearse_reservation_upgrade(self):
        with TemporaryDirectory() as temporary_directory:
            root = Path(temporary_directory)
            source_path = root / "order-foundation.db"
            backup_directory = root / "backups"
            create_baseline_database(source_path)
            database_url = build_sqlite_database_url(source_path)
            command.upgrade(build_config(database_url), "0007_order_foundation")
            source_snapshot = capture_legacy_snapshot(source_path)
            ...
            self.assertEqual(result.source_revision, "0007_order_foundation")
            self.assertEqual(
                get_current_revision(source_path),
                "0007_order_foundation",
            )
            self.assertEqual(
                get_current_revision(result.rehearsal_database),
                CURRENT_SCHEMA_REVISION,
    ...
    def test_order_reservation_revision_can_rehearse_lifecycle_upgrade(self):
        with TemporaryDirectory() as temporary_directory:
            root = Path(temporary_directory)
            source_path = root / "order-reservation.db"
            backup_directory = root / "backups"
            create_baseline_database(source_path)
            database_url = build_sqlite_database_url(source_path)
            command.upgrade(
                build_config(database_url),
                "0008_order_reservation",
            )
            source_snapshot = capture_legacy_snapshot(source_path)
            ...
            self.assertEqual(result.source_revision, "0008_order_reservation")
            self.assertEqual(
                get_current_revision(source_path),
                "0008_order_reservation",
            )
    ...
    def test_order_refund_revision_can_rehearse_settlement_upgrade(self):
        with TemporaryDirectory() as temporary_directory:
            root = Path(temporary_directory)
            source_path = root / "order-refund.db"
            backup_directory = root / "backups"
            create_baseline_database(source_path)
            database_url = build_sqlite_database_url(source_path)
            command.upgrade(

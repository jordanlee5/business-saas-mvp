...
- 超级管理员与运营管理员可管理商品、SKU、库存、订单和供应商，生成待确认的供应商结算，并导出已确认结算；
...
- M6-4 新增供应商结算批次只读列表与详情服务，并为已确认批次生成包含汇总和订单项成本快照的 Excel；导出仅限启用中的超级管理员和运营管理员且逐次记录审计，完整边界见 [供应商结算查询与导出说明](docs/supplier-settlement-export.md)；
- M6-5 在 `/mall-settlements` 提供受控后台列表、详情和已确认批次下载，读取仍执行证据复核，下载仍调用 M6-4 原子导出并逐次审计，完整边界见 [供应商结算后台只读入口说明](docs/supplier-settlement-admin.md)；
- M6-6 在同一列表页为启用中的运营管理员提供结算生成表单，提交时先验证账号与表单令牌，再调用 M6-2 原子生成服务；无合格订单项不创建空批次，成功后跳转详情，边界见 [结算生成后台入口说明](docs/supplier-settlement-generation-admin.md)；
...
- 商品/库存 Excel、订单写操作页面、小程序下单 API、部分退款、异常人工恢复，以及结算撤销、支付和冲销仍按后续切片独立实现。
...
- 凭证、宣传页图片及商品图片保存在本地 `uploads/` 目录；商品图片使用独立的 `uploads/mall_products/` 子目录；
...
python -m unittest -v test_order_refund_migration.py test_supplier_settlement_migration.py
python -m unittest -v test_supplier_settlement_service.py
...
python -m unittest -v test_postgresql_supplier_settlement.py
```
...
已经稳定停留在 `0001_current_schema_baseline` 至 `0010_order_refund_recovery` 之间任一受支持版本的 SQLite 数据库，在升级到当前结构前必须停止应用，并运行：

```powershell
python -m app.migration_upgrade_rehearsal
```

该命令会在 `database_backups/mall_core_upgrade_rehearsals/` 保存升级前快照和独立演练副本，只对演练副本重复执行 `upgrade head` 与结构漂移检查。它逐表记录源版本的原字段定义、行数和字段值指纹，确认升级后全部不变，并核验演练副本真实处于 `0011_supplier_settlement_foundation`。源库版本和业务指纹保持不变才会通过。审阅输出并按需使用演练副本完成验证后，才可另行批准对真实库执行 `python -m alembic -c alembic.ini upgrade head`；数据库文件与 `uploads/` 备份必须继续保留。

## 测试基线

在当前 M6-8 工作副本上，完整依赖环境中的回归命令为：
...
- Python 静态编译：通过；
- M6-8 专项及关联回归 `Ran 70 tests`、`OK`，全量回归 `Ran 600 tests`、`OK (skipped=4)`；4 个 PostgreSQL 实连接集成类需在独立测试数据库中另行验收；
...
│  ├─ supplier-settlement-export.md # M6-4 结算查询与审计导出边界

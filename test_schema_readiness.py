...
class SchemaReadinessTests(unittest.TestCase):
    ...
    def test_false_head_missing_order_snapshot_table_is_rejected(self):
        with TemporaryDirectory() as temporary_directory:
            database_url = build_sqlite_url(
                Path(temporary_directory) / "missing-order-items.db"
            )
            upgrade(database_url, "head")
            engine = create_engine(database_url)
            try:
                with engine.begin() as connection:
                    connection.execute(text("DROP TABLE order_items"))
